(() => {
  ...
  const messages = {
    tr: {
      ...
      welcomeKicker: "İLK KEZ BURADA MISIN?", welcomeTitle: "Hoş geldin, {name}.", welcomeBody: "Game Compass, sevdiğin oyun sistemlerini anlayıp keşif akışını onlara göre şekillendirir. Önce kısa bir profil oluştur, sonra pusulanı kullanmaya başla.", dayGame: "GÜNÜN ÖNERİSİ", continue: "Keşfe geç", startGuide: "Rehberli turu başlat", viewDossier: "Künye aç", noDayGame: "Bugün için uygun bir oyun bulunamadı.",
    ...
    en: {
      ...
      welcomeKicker: "FIRST TIME HERE?", welcomeTitle: "Welcome, {name}.", welcomeBody: "Game Compass learns the game systems you enjoy and shapes discovery around them. Create a short profile first, then start exploring.", dayGame: "GAME OF THE DAY", continue: "Enter discovery", startGuide: "Start guided tour", viewDossier: "Open dossier", noDayGame: "No suitable game was found for today.",
  ...
  function renderWelcome() {
    ...
    const candidates = allCatalogGames().filter((game) => !game.owned && !isBlocked(game)).sort((a, b) => profileFit(b) - profileFit(a) || (b.reviewPercent || 0) - (a.reviewPercent || 0)); const game = candidates[0];
  ...
  function installHandlers() {
    ...
    document.addEventListener("click", (event) => {
      ...
      const start = event.target.closest("[data-gc-start-guide], #guideButton"); if (start) { event.preventDefault(); event.stopImmediatePropagation(); startGuide(); return; }
